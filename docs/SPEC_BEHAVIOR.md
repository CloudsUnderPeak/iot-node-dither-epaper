...
- 公開唯讀的 `GET /api/device` 在 `diagnostics.reset_reason` 回報導致目前 boot 的前一次 reset 原因。Firmware 在 boot 只讀取一次並保存 immutable snapshot；值固定為 `power_on`、`external`、`software`、`panic`、`watchdog`、`deep_sleep`、`brownout` 或 `unknown`。本功能不建立歷史、不保存一般 log、不讀 coredump、不寫 NVS／filesystem，也不從 reset 分類推測 wake source 或其他未提供的原因。
- 電池狀態沿用公開唯讀的 `GET /api/device`，在 `power.voltage_mv`、`power.sample_age_ms` 與 `power.estimated_percent` 直接回報 GPIO0 板載分壓量得的電池電壓與由單節鋰電池電壓曲線推算的估計百分比，不另設 `/api/power`，也不保留舊的 `power.battery` compatibility alias。本功能不修改硬體，無法辨識是否安裝電池、外部電源或充電狀態；response 不提供 `charge_state`、`charging`、`battery_present`、`usb_present` 或 `power_source`，也不得用電壓變化推測並宣稱正在充電。
