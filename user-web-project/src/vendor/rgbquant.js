...
//		this.sortPal = ['hue-','lum-','sat-'];
		...
		// Embedded Web Dithering extension: percent-based error diffusion strength.
		// `dithStrength` remains as a multiplier alias for earlier local builds.
		var errorDiffusionStrength = typeof opts.errorDiffusionStrength == "number"
			? opts.errorDiffusionStrength / 100
			: opts.dithStrength;
		this.errorDiffusionStrength = typeof errorDiffusionStrength == "number" ? errorDiffusionStrength : 1;
			...
			for (var x = (dir == 1 ? 0 : width - 1), xend = (dir == 1 ? width : 0); x !== xend; x += dir) {
				...
				for (var i = (dir == 1 ? 0 : ds.length - 1), end = (dir == 1 ? ds.length : 0); i !== end; i += dir) {
					...
					if (x1 + x >= 0 && x1 + x < width && y1 + y >= 0 && y1 + y < height) {
						var d = ds[i][0] * this.errorDiffusionStrength;
